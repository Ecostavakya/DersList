...
public class FileService {
    ...
    public  List<String> getDersIsimleri(String path) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(path))) {
            ...
            while ((line = reader.readLine()) != null) {
                // Parçalama işlemi - Burada veriyi parse ederek ders isimlerini alıyoruz
    ...
    public void writeToFile2(String path, String fileName, String data1, String data2 , String data3 , String data4) {
        CreateFile(path, fileName);
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(path + "/" + fileName, true))) {
            writer.write("Ogrenci Adı: " + data1 + ", Ogrenci Soyadı: " + data2 + ", Ogrenci No: " + data3 + ", Ogrenci Ders: " + data4); // Concatenate data if needed
            writer.newLine(); // Add a new line
        } catch (IOException e) {
            e.printStackTrace(); // Handle the exception as per your application's requirements
        }
    ...
    public static void main(String[] args) {
        String filePath = "src/Data/Ders.txt"; // Dosya yolunu güncelleyin
        FileService fileService= new FileService();
        List<String> dersIsimleri = fileService.getDersIsimleri(filePath);

        // Ders isimlerini ekrana yazdır
